...
LmL0 = function(
    ...
    out$Beta = Matrix::Matrix(
        out$Beta[, 1:nlambdai, drop = F] * sdy,
        sparse = TRUE
    )
    out$BetaSTD = Matrix::Matrix(
        out$BetaSTD[, 1:nlambdai, drop = F] * sdy,
        sparse = TRUE
    )
    ...
    if (nfolds == 1 & is.null(foldid)) {
        ...
        outi = list()
        cvRSS = matrix(NA, nrow = nfolds, ncol = nlambdai)
        for (i in 1:nfolds) {
            temid = (foldid == i)
        ...
        if (aPen) {
            ...
            cuti = ifelse(
                length(temi) > sum(wbeta == 0),
                which.min(temi[-c(1:sum(wbeta == 0))]) + sum(wbeta == 0),
                sum(wbeta == 0)
            )
        }
