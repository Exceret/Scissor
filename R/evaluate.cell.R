...
evaluate.cell <- function(
    ...
    evaluate_summary <- as.data.frame(matrix(
        0,
        n,
        11,
        dimnames = list(
    ...
    cli::cli_progress_bar(
        "Performing correlation check for each selected cell",
        total = m
    )
    ...
    for (i in 1:m) {
        ...
        Sys.sleep(1 / 100)
        for (j in 1:n) {
            cor_test_p[i, j] <- stats::cor.test(
                Expression_bulk[, i],
                Expression_cell[, selected_cell[j]]
            )$p.value
        }
    ...
    cor_test_FDR <- matrix(
        stats::p.adjust(as.numeric(cor_test_p), method = "fdr"),
        m
    )
    for (j in 1:n) {
        evaluate_summary[j, 1] <- Matrix::mean(X[, selected_cell[j]])
        evaluate_summary[j, 2] <- percent(sum(X[, selected_cell[j]] > 0) / m)
        evaluate_summary[j, 3] <- percent(sum(X[, selected_cell[j]] < 0) / m)
        evaluate_summary[j, 4] <- percent(
            sum(cor_test_FDR[, j] < FDR_cutoff) / m
        )
    }
    ...
    Matrix::print("|**************************************************|")
    cli::cli_progress_bar(
        "Performing nonparametric bootstrap",
        total = bootstrap_n
    )
    beta_bootstrap <- matrix(
        0,
        bootstrap_n,
        n,
        dimnames = list(paste0("Bootstrap_", 1:bootstrap_n), selected_cell)
    )
    ...
    for (j in 1:n) {
        ...
        evaluate_summary[j, 6:10] <- stats::fivenum(tmp)
