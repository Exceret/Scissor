...
CoxL0 = function(
  ...
  if (nrow(y) == 0 || ncol(y) < 2) {
    stop("Input survival data y is invalid")
  }
  ...
  if (nfolds == 1 & is.null(foldid)) {
    ...
    if (any(temi > 0)) {
      nlambdai = max(min(Matrix::which(temi > 0)) - 1, 1)
    }
    ...
    indexij = Matrix::which(cvm >= (cvm[indexi] - cvse[indexi]))[1]
    temi = rep("", nlambdai)
      ...
      if (numi2 > 0) {
        ...
        for (i in 1:nfolds) {
          ...
          if (numj == 0) {
            ...
            temo = data.frame(
              temo[Matrix::which(temo <= numj)],
              Matrix::which(temo <= numj)
            )
            ...
            if (numi2 > 0) {
              ...
              for (i in 1:nfolds) {
                ...
                if (numj == 0) {
                  ...
                  BetaSTDjj = BetaSTDj
                  BetaSTDjj[wbeta == 0] = max(abs(BetaSTDj)) +
                    1
                  temo = rank(
                    -abs(BetaSTDjj),
                    ties.method = "min"
    ...
    Beta0j[Matrix::which(wbeta == 0)] = max(abs(Beta0j)) + 1
...
PrepCox = function(x, y) {
  ...
  if (N0 == 0 || ncol(y) < 2) {
    stop("Invalid input: x or y is empty or malformed")
  }
  ...
  i1 = base::which(y[, "status"] == 1)

  if (length(i1) == 0) {
    warning("No events in the data")
  }

  mi1 = min(i1) - 1
  if (mi1 != 0) {
    x = x[-c(1:mi1), , drop = FALSE]
    y = y[-c(1:mi1), , drop = FALSE]
  }

  if (nrow(x) == 0 || nrow(y) == 0) {
    warning("All observations removed during preprocessing")
  }
  ...
  if (any(dty)) {
    tevent0 = tevent
    tevent0[base::which(dty)] = 0

    ievent = cumsum(tevent0)
    loc1 = base::which(tevent0 == 1)

    if (length(loc1) == 0) {
      warning("No unique events found")
    }
