...
LogL0 = function(
    ...
    if (length(indexi) > 0) {
        ...
        wbeta1i = wbeta1i[-indexi]
        wbetaii = wbetaii[-indexi]
    }
    ...
    if (nfolds == 1 & is.null(foldid)) {
        ...
        repeat {
            ...
            cv.min[il0] = ifelse(
                length(temi) > sum(wbeta1 == 0),
                min(temi[-c(1:sum(wbeta1 == 0))]),
                temi[sum(wbeta1 == 0)]
            )
        ...
        cuti = ifelse(
            length(temi) > sum(wbeta1 == 0),
            which.min(temi[-c(1:sum(wbeta1 == 0))]) + sum(wbeta1 == 0),
            sum(wbeta1 == 0)
        )
        # cuti=which.min(cvm[[index0]])

        Beta0j = out$BetaSTD[, index0]
        Beta0j[Matrix::which(wbeta1 == 0)] = max(abs(Beta0j)) + 1
