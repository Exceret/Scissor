...
#' @param ... print `verbose` message, use `seed` for reproducibility and other optional arguments. Use `assay` to specify the assay slot of the Seurat object.
#'
#'
#' @references
...
Scissor.v5.optimized <- function(
  ...
  if (is.null(Load_file)) {
    ...
    if (graph_name %chin% names(sc_dataset@graphs)) {
      ...
      cli::cli_abort(c(
        "x" = "No {.arg {graph_name}} graph in the given Seurat object. Please check Scissor inputs."
      ))
    }
