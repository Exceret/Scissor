...
List OmegaC(Eigen::MatrixXd &Omega, Eigen::VectorXi &sgn) {
  ...
  for (i = 0; i < p; ++i) {
    ...
    for (Eigen::SparseMatrix<double>::InnerIterator it(OmegaS, i); it; ++it) {
      loc(j++, i) = it.index();
      OmegaS.coeffRef(it.index(), i) = it.value() * sgn(i) * sgn(it.index()) /
                                       sqrt(ndegree(i) * ndegree(it.index()));
    }
  }

  return (List::create(Named("nadj") = nadj, Named("loc") = loc,
                       Named("Omega") = OmegaS));
}
...
// [[Rcpp::export]]
double maxLambdaLmC(Eigen::MatrixXd X, Eigen::VectorXd y, double alpha,
                    Eigen::VectorXd wbeta, int N0, int p) {
  int i;
...
// [[Rcpp::export]]
Eigen::VectorXd cvTrimLmC(Eigen::VectorXd beta, int nn, int nn2,
                          Eigen::VectorXi loco, Eigen::MatrixXd XF,
                          Eigen::VectorXd yF, int NF, double a0) {
  int i, j;
  Eigen::VectorXd RSS,
      xbF = Eigen::VectorXd::Zero(NF); // xb=Eigen::VectorXd::Zero(N),
...
/*****  LM: Enet (L1+L2)  *****/
// [[Rcpp::export]]
List EnetLmC(Eigen::MatrixXd X, Eigen::VectorXd y, double alpha,
             Eigen::VectorXd lambda, int nlambda, int ilambda,
             Eigen::VectorXd wbeta, int p, int N0, double thresh, int maxit,
             double thresh2) {
  ...
  Eigen::MatrixXd Beta = Eigen::MatrixXd::Zero(p, nlambda),
                  BetaSTD = Eigen::MatrixXd::Zero(p, nlambda);
  ...
  Eigen::VectorXd RSS = Eigen::VectorXd::Zero(nlambda),
                  RSQ = Eigen::VectorXd::Zero(nlambda);
  double xr, dbMax;
  ...
  for (il = 0; il < nlambda; ++il) {
    ...
    lambda2 =
        lambda(il) *
        (1.0 - alpha); // lambda1:vector lambda*alpha, lambda2=lambda*(1-alpha)
  ...
  local:
    while (1) {
      ...
      if (std::abs(rss0 - rss1) < std::abs(thresh2 * rss1)) {
        flag(il) = 0;
        break;
      }
      if (rss0 < RSS0 * 0.001) {
      ...
      if (std::abs(obj1 - obj0) < std::abs(thresh2 * obj1)) {
        flag(il) = 0;
        break;
      }
      if (obj0 != obj0) {
...
exit:
  return (List::create(Named("Beta") = Beta, Named("BetaSTD") = BetaSTD,
                       Named("flag") = flag, Named("rsq") = RSQ,
                       Named("RSS") = RSS, Named("lambda") = lambda,
                       Named("nlambda") = il));
}

/*****  LM: Enet (L1+L2) cross-validation  *****/
// [[Rcpp::export]]
List cvEnetLmC(Eigen::MatrixXd X, Eigen::VectorXd y, double alpha,
               Eigen::VectorXd lambda, int nlambda, Eigen::VectorXd wbeta,
               int N, int p, double thresh, int maxit, Eigen::MatrixXd XF,
               Eigen::VectorXd yF, int NF, double thresh2) {

  int i, j, it = 0, il, iadd, ia = 0;
  double lambda2, zi, obj0, obj1, rss0, rss1, b0, db0, objQi, objQj, RSS0;
  Eigen::VectorXd beta0 = Eigen::VectorXd::Zero(p);
  Eigen::MatrixXd Beta = Eigen::MatrixXd::Zero(p, nlambda),
                  BetaSTD = Eigen::MatrixXd::Zero(
                      p, nlambda); // beta matrix for different lambdas
  Eigen::VectorXd lambda1(p);
...
exit:
  return (List::create(
      Named("Beta") = Beta, Named("BetaSTD") = BetaSTD, Named("flag") = flag,
      Named("predY") = predY, Named("a0S") = a0S, Named("RSS") = RSS,
      Named("rsq") = RSQ, Named("RSSp") = RSSp, Named("nlambda") = il));
}
...
List NetLmC(Eigen::MatrixXd &X, Eigen::VectorXd &y, double alpha,
            Eigen::VectorXd lambda, int nlambda, int ilambda,
            Eigen::VectorXd wbeta, Eigen::SparseMatrix<double> &Omega,
            Eigen::MatrixXd loc, Eigen::VectorXi nadj, int p, int N0,
            double thresh, int maxit, double thresh2) {

  int i, j, ij, m, it = 0, il, iadd, ia = 0;
  double lambda2, zi, zi2, objQi = 0.0, objQj, obj0, obj1, rss0, rss1, b0, db0,
                           RSS0;
  Eigen::VectorXd beta0 = Eigen::VectorXd::Zero(p);
  Eigen::MatrixXd Beta = Eigen::MatrixXd::Zero(p, nlambda),
                  BetaSTD = Eigen::MatrixXd::Zero(p, nlambda);
    ...
    while (1) {
      ...
      for (i = 0; i < ia; ++i) {
        ...
        for (ij = 0; ij < nadj(j); ++ij) {
          m = loc(ij, j) - 1;
          if (iactive(m) == 1) {
            zi2 += beta0(m) * Omega.coeffRef(m, j);
          } // Omega: w_kl/sqrt(d_k*d_l),L=I-Omega ; L=SLS (included sign of
            // beta)
        }
        zi += lambda2 * zi2;
      ...
      if (std::abs(rss1 - rss0) < std::abs(thresh2 * rss1)) {
        flag(il) = 0;
        break;
      }
      if (rss0 < RSS0 * 0.001) {
    ...
    for (i = 0; i < p; ++i) {
      if (iactive(i) == 0) {
        ...
        for (ij = 0; ij < nadj(i); ++ij) {
          m = loc(ij, i);
          if (iactive(m) == 1) {
            zi2 += beta0(m) * Omega.coeffRef(m, i);
          }
        }
...
List cvNetLmC(Eigen::MatrixXd &X, Eigen::VectorXd &y, double alpha,
              Eigen::VectorXd lambda, int nlambda, Eigen::VectorXd wbeta,
              Eigen::SparseMatrix<double> &Omega, Eigen::MatrixXd loc,
              Eigen::VectorXi nadj, int N, int p, double thresh, int maxit,
              Eigen::MatrixXd XF, Eigen::VectorXd yF, int NF, double thresh2) {
...
exit:
  return (List::create(
      Named("Beta") = Beta, Named("BetaSTD") = BetaSTD, Named("flag") = flag,
      Named("predY") = predY, Named("a0S") = a0S, Named("RSS") = RSS,
      Named("RSSp") = RSSp, Named("rsq") = RSQ, Named("nlambda") = il));
}
...
// [[Rcpp::export]]
Eigen::VectorXd cvTrimCoxC(Eigen::VectorXd beta, int nn, int nn2,
                           Eigen::VectorXi loco, Eigen::MatrixXd XF, int NF,
                           Eigen::VectorXi neventF, Eigen::VectorXi nevent1F,
                           Eigen::VectorXi loc1F, int nF, Eigen::MatrixXd X,
                           int N, Eigen::VectorXi nevent,
...
/*****  Cox: Enet (L1+L2)  *****/
// [[Rcpp::export]]
List EnetCoxC(Eigen::MatrixXd X, Eigen::VectorXd tevent, double alpha,
              Eigen::VectorXd lambda, int nlambda, int ilambda,
              Eigen::VectorXd wbeta, int N, Eigen::VectorXi nevent,
              Eigen::VectorXi nevent1, Eigen::VectorXi loc1, int n, int p,
              int N0, double thresh, int maxit, int ifast) {

  int i, j, it, il, iadd, ia = 0, itwo = 0;
  double lambda2, lambda2i, zi, obj0, obj1, ll0, ll1, b0, db0, PLi, PLi2, objQi,
      objQj;
  Eigen::VectorXd beta0 = Eigen::VectorXd::Zero(p);
  Eigen::MatrixXd Betas = Eigen::MatrixXd::Zero(p, nlambda),
                  BetasSTD = Eigen::MatrixXd::Zero(p, nlambda);
  ...
  if (ilambda == 1) {
    if (alpha > 0.0) {
      lambdaMax = maxLambdaCoxC(X, tevent, N, nevent, nevent1, loc1, n, alpha,
                                wbeta, N0, p);
    } else {
      lambdaMax = maxLambdaCoxC(X, tevent, N, nevent, nevent1, loc1, n, 0.001,
                                wbeta, N0, p);
    }
    lambda = lambda.array() * lambdaMax;
  }
...
exit:
  if (ifast == 1 && itwo == 1 && il > 0)
    --il;
  return (List::create(Named("Beta") = Betas, Named("BetaSTD") = BetasSTD,
                       Named("flag") = flag, Named("lambda") = lambda,
                       Named("ll") = locbeta, Named("nlambda") = il));
}

/*****  Cox: Enet (L1+L2) cross-validation  *****/
// [[Rcpp::export]]
List cvEnetCoxC(Eigen::MatrixXd X, Eigen::VectorXd tevent, double alpha,
                Eigen::VectorXd lambda, int nlambda, Eigen::VectorXd wbeta,
  ...
  local:
    ...
    for (i = 0; i < ia; i++) {
      j = active(i);
      xbF += XF.col(j) * Betas(j, il);
    }
    exbF = (xbF.array()).exp();
    locbetaF(il) =
        pletaCm(xbF, exbF, neventF, nevent1F, loc1F, nF, ifast, itwo);
  } // for lambda
...
exit:
  if (ifast == 1 && itwo == 1 && il > 0)
    --il;
  return (List::create(Named("Beta") = Betas, Named("BetaSTD") = BetasSTD,
                       Named("flag") = flag, Named("ll") = locbeta,
...
List NetCoxC(Eigen::MatrixXd &X, Eigen::VectorXd tevent, double alpha,
             Eigen::VectorXd lambda, int nlambda, int ilambda,
             Eigen::VectorXd wbeta, Eigen::SparseMatrix<double> &Omega,
             Eigen::MatrixXd loc, Eigen::VectorXi nadj, int N,
             Eigen::VectorXi nevent, Eigen::VectorXi nevent1,
             Eigen::VectorXi loc1, int n, int p, int N0, double thresh,
             int maxit, int ifast) {

  int i, j, ij, m, it, il, iadd, ia = 0, itwo = 0;
  double lambda2, lambda2i, zi, zi2, objQi = 0.0, objQj, obj0, obj1, ll0, ll1,
                                     b0, db0, PLi, PLi2;
  Eigen::VectorXd beta0 = Eigen::VectorXd::Zero(p);
  Eigen::MatrixXd Betas = Eigen::MatrixXd::Zero(p, nlambda),
                  BetasSTD = Eigen::MatrixXd::Zero(p, nlambda);
...
List cvNetCoxC(Eigen::MatrixXd &X, Eigen::VectorXd tevent, double alpha,
               Eigen::VectorXd lambda, int nlambda, Eigen::VectorXd wbeta,
               Eigen::SparseMatrix<double> &Omega, Eigen::MatrixXd loc,
               Eigen::VectorXi nadj, int N, Eigen::VectorXi nevent,
               Eigen::VectorXi nevent1, Eigen::VectorXi loc1, int n, int p,
               int N0, double thresh, int maxit, int ifast, Eigen::MatrixXd XF,
               int NF, Eigen::VectorXi neventF, Eigen::VectorXi nevent1F,
               Eigen::VectorXi loc1F, int nF) {
...
// [[Rcpp::export]]
double maxLambdaLogC(Eigen::MatrixXd X, Eigen::VectorXd Z, double alpha,
                     Eigen::VectorXd wbeta, int N0, int p) {
  int i;
  double LiMax = 0.0, LiMaxi = 0.0;
...
/*****  Used for CV trimming  *****/
// [[Rcpp::export]]
Eigen::VectorXd cvTrimLogC(Eigen::VectorXd beta, int nn, int nn2,
                           Eigen::VectorXi loco, Eigen::MatrixXd XF,
                           Eigen::VectorXd yF, int NF, double threshP) {
  int i, j;
  Eigen::VectorXd Dev(nn2),
      xbF = Eigen::VectorXd::Zero(NF); // xb=Eigen::VectorXd::Zero(N),
  Eigen::ArrayXd p0(NF);
  ...
  for (i = 0; i < nn; i++) {
    ...
    Dev(i) =
        (yF.array() * p0.log() + (1.0 - yF.array()) * (1.0 - p0).log()).sum() *
        (-2.0);
  }
...
/*****  Log: Enet (L1+L2)  *****/
// [[Rcpp::export]]
List EnetLogC(Eigen::MatrixXd X, Eigen::VectorXd y, double alpha,
              Eigen::VectorXd lambda, int nlambda, int ilambda,
              Eigen::ArrayXd wbeta, Eigen::ArrayXd wbetai, int p, int N0,
              double thresh, int maxit, double threshP) {
  ...
  for (il = 0; il < nlambda; ++il) {
    lambda1 = lambda(il) * alpha * wbeta * wbetai;
    lambda2 = lambda(il) * (1.0 - alpha) *
              wbetai; // lambda1:vector lambda*alpha, lambda2=lambda*(1-alpha)
  ...
  local:
    ...
    LL(il) =
        (y.array() * p0.log() + (1.0 - y.array()) * (1.0 - p0).log()).mean();
...
// [[Rcpp::export]]
List cvEnetLogC(Eigen::MatrixXd X, Eigen::VectorXd y, double alpha,
                Eigen::VectorXd lambda, int nlambda, Eigen::ArrayXd wbeta,
                Eigen::ArrayXd wbetai, int p, int N0, double thresh, int maxit,
  ...
  local:
    ...
    LLF(il) = (yF.array() * pF0.log() + (1.0 - yF.array()) * (1.0 - pF0).log())
                  .mean();
  ...
  llF0 =
      (yF.array() * pF0.log() + (1.0 - yF.array()) * (1.0 - pF0).log()).mean();
...
/*****  Log: Network (L1+La)  *****/
// [[Rcpp::export]]
List NetLogC(Eigen::MatrixXd X, Eigen::VectorXd y, double alpha,
             Eigen::VectorXd lambda, int nlambda, int ilambda,
             Eigen::ArrayXd wbeta, Eigen::ArrayXd wbetai,
             Eigen::SparseMatrix<double> &Omega, Eigen::MatrixXd loc,
             Eigen::VectorXi nadj, int p, int N0, double thresh, int maxit,
             double threshP) {
...
/*****  Log: Enet (L1+L2) cross-validation  *****/
// [[Rcpp::export]]
List cvNetLogC(Eigen::MatrixXd X, Eigen::VectorXd y, double alpha,
               Eigen::VectorXd lambda, int nlambda, Eigen::ArrayXd wbeta,
               Eigen::ArrayXd wbetai, Eigen::SparseMatrix<double> &Omega,
               Eigen::MatrixXd loc, Eigen::VectorXi nadj, int p, int N0,
               double thresh, int maxit, Eigen::MatrixXd XF, Eigen::VectorXd yF,
               int NF, double threshP) {
